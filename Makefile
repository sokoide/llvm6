...
TARGET = ./ccompiler_c
CC = gcc
CFLAGS = -Wall -Wextra -O2 -std=c99 -Isrc -DTC1
LIBS =
...
# Sources for bootstrapping
TC_SRCS = src/memory.c src/error.c src/ast.c src/symbols.c src/codegen.c src/main.c
TC_GEN_SRCS = $(BUILD_DIR)/grammar_c.tab.c $(BUILD_DIR)/lex_c.yy.c

# IR files generated by TC1
TC2_LL = $(TC_SRCS:src/%.c=$(BOOTSTRAP_DIR)/%.ll) \
         $(BOOTSTRAP_DIR)/grammar_c.tab.ll \
         $(BOOTSTRAP_DIR)/lex_c.yy.ll
...
clean-bootstrap:
	rm -rf $(BOOTSTRAP_DIR) $(TC2) $(TC3)

clean:
	rm -rf $(BUILD_DIR) $(TARGET)

.PHONY: all clean test test-unit test-integration bootstrap clean-bootstrap
