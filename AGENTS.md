...
The core compiler lives in `srccpp/` (C++ version) and `src/` (Pure C version), split into `lexer.l`, `grammar.y`, `ast.*`, `codegen.*`, and `main.cpp/c`. Generated artifacts and object files land in `build/`. Documentation lives under `docs/`. Tests are split into `testscpp/` (C++ unit tests) and `tests/` (Shared fixtures and C unit tests). Avoid committing anything from `build/` or `tests/output/`; they are reproducible.
