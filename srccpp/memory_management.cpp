...
MemoryContext* create_memory_context(void) {
    auto ctx = static_cast<MemoryContext*>(malloc(sizeof(MemoryContext)));
    if (!ctx) {
...
void free_memory_context(MemoryContext* ctx) {
    ...
    while (block) {
        ...
        fprintf(stderr,
                "Memory leak detected: %zu bytes allocated at %s:%d in %s()\n",
...
void* safe_malloc_debug(size_t size, const char* file, int line,
    ...
    if (!ptr) {
        fprintf(stderr,
                "Error: Memory allocation failed for %zu bytes at %s:%d in "
                "%s()\n",
    ...
    if (g_memory_context->stats.current_usage >
        g_memory_context->stats.peak_usage) {
        g_memory_context->stats.peak_usage =
            g_memory_context->stats.current_usage;
    }
...
char* safe_strdup_debug(const char* str, const char* file, int line,
    ...
    auto new_str =
        static_cast<char*>(safe_malloc_debug(len, file, line, function));
    strcpy(new_str, str);
    return new_str;
}
...
void safe_free_debug(void* ptr, const char* file, int line,
    ...
    if (!g_memory_context) {
        fprintf(stderr,
                "Warning: Freeing memory without memory context at %s:%d in "
                "%s()\n",
                file, line, function);
        free(ptr);
        return;
...
void print_memory_leaks(const MemoryContext* ctx) {
    ...
    if (ctx->debug_mode && ctx->allocated_blocks) {
        ...
        const MemoryBlock* block = ctx->allocated_blocks;
        while (block) {
